...
if (!isset($_GET['csvfile']) && !isset($_POST['selection'])) {
    ...
    $menSize = $SIZE;
    //Replacing the Space For "~" to have a reference point to split the size in two
    $SIZE = str_replace(' ', '~', $menSize);

    // Assignamos los colores PMS a cada Talla
    if($SIZE == 'XS/XCH~(4-5)') {
        $menFillColor = $pms_185C;
    } elseif($SIZE == 'S/CH~(6-7)'){
    ...
    for ($i = 1; $i <= 2; $i++) {
        $fontColor = ($SIZE == 'M~(8)') ? $black : $white; //this sets the font color to black if Size = L 42/44
