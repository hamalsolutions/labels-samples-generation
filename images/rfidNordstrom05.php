...
if (!isset($_GET['csvfile']) && !isset($_POST['selection'])) {
	...
	$logo = fuente('GoodAmericanLogo.ttf');
	...
	If (strlen($STYLE) < 10) {
		texto($STYLE, 12, 106, 0, 0, $arialBold, $black, 8, 0, 0);
	} else {
		texto($STYLE, 12, 106, 0, 0, $arialNarrowBold, $black, 8, 0, 0);
	}

	If (strlen($COLOR) < 18) {
		texto($COLOR, 12, 106, 2, 12, $arialBold, $black, 8, 0, 0);
	} else {
		texto($COLOR, 12, 106, 2, 12, $arialNarrowBold, $black, 8, 0, 0);
	}

	texto($COLOR_NRF.'      '.$COLOR_CODE, 0, 122, 1, 0, $arialBold, $black, 8, 0, 0);

	If (strlen($DESCRIPTION) < 17) {
		texto($DESCRIPTION, 0, 130, 1, 0, $arialBold, $black, 8, 0, 0);
	} else {
		texto($DESCRIPTION, 0, 138, 1, 0, $arial70B, $black, 8, 0, 0);
	}

	texto($SIZE, 0, 252, 1, 0, $arialBold, $black, 14, 0, 0);
	//perforacion(150,325,300);
	if ($MSRP <> '') {
		texto('MSRP', 16, 316, 0, 0, $arialBold, $black, 10, 0, 0);
	}

	texto($MSRP, 0, 316, 2, 18, $arialNarrowBold, $black, 11, 0, 1);
