       ...
       { 
        ...
        if ($sample == 2)
        {
            ...
            if ($so == false)
                $so = '0235488';
            if ($po == false)
                $po = '40046, 58421, 5841, WJN78';
            if ($via == false)
                $via = 'Will Call';
            if ($description == false)
                $description = 'PRINTED CARELABEL 3323 LASER ECONO SHEETED 1 1/3 X 2 3/4';
            if ($qty == false)
                $qty = '2,000';
            if ($boxI == false)
                $boxI = '2';
        ...
        imagettftext($img,15,0,430,20,$text_color,$arialBold,'ORDER #:');
        imagettftext($img,18,0,445,42,empty($so)?$m_text_color:$text_color,$arialBold,empty($so)?$mtext:$so);
        ...
        imagettftext($img,15,0,430,75,$text_color,$arialBold,"# OF BOXES:");
        texto($boxI, 0, 110, 3, -300, $arialBold, $text_color, 29, 0, 0);
        //imagettftext($img,29,0,430,110,empty($boxI)?$m_text_color:$text_color,$arialBold,empty($boxI)?$mtext:$boxI);
        imagettftext($img,15,0,480,110,$text_color,$arialBold,'OF');
        texto($boxT, 0, 110, 3, -470, $arialBold, $text_color, 29, 0, 0);
        //imagettftext($img,29,0,510,110,empty($boxI)?$m_text_color:$text_color,$arialBold,empty($boxI)?$mtext:$boxT);
        
        imagettftext($img,15,0,190,180,$text_color,$arialBold,"SHIP TO:");
        
        imagettftext($img,14,0,200,210,$text_color,$arialBold,$dcompany);
        parrafo($daddress, 200, 232, 0, 0, $arialBold, $text_color, strlen($daddress)>15?13:14, 0, 40, 15);
        //imagettftext($img,14,0,200,232,$text_color,$arialBold,$daddress);
        $dcolonia = '';
        if(empty($dcolonia))
        {
            imagettftext($img,14,0,200,252,$text_color,$arialBold,$dcity.','.$dstate.' '.$dzip);
            imagettftext($img,14,0,200,282,$text_color,$arialBold,'ATTN: '.$dattn);
        }
        else
        {
            imagettftext($img,14,0,200,252,$text_color,$arialBold,$dcolonia);
            imagettftext($img,14,0,200,277,$text_color,$arialBold,$dcity.','.$dstate.' '.$dzip);
            imagettftext($img,14,0,200,307,$text_color,$arialBold,'ATTN: '.$dattn);
        }
        
        texto('SHIP VIA: '.$via, 200, 365, 0, 0, $arialBold, $text_color, 15, 0, 0);
        
        parrafo('PO# '.$po, 12, 200, 0, 0, $arialNarrowBold, $text_color, 12, 0, 18, 20);
        ...
        parrafo($description, 12, 265, 0, 0, $arialNarrowBold, $text_color, 12, 0, 18, 20);
        //imagettftext($img,15,0,30,268,empty($via)?$m_text_color:$text_color,$arialBold,empty($via)?$mtext:$via);
        ...
        texto($qty, 0, 375, 2, 450, $arialBold, $text_color, 20, 0, 0);
        //imagettftext($img,15,0,30,358,empty($qty)?$m_text_color:$text_color,$arialBold,empty($qty)?$mtext:$qty);        
       ...
       else
       {
           ...
           for($i=1;$i<=$qty;$i++)
           {
               if ($orderInfo['shipping_method_carrier']<>'Will Call')
               {
                   ...
                   $addressInfo['international'] = str_replace("#","%23",$addressInfo['international']);
                   $addressInfo['company'] = str_replace(" & "," %26 ",$addressInfo['company']);
