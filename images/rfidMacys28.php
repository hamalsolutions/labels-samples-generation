...
if (!isset($_GET['csvfile']) && !isset($_POST['selection'])) {
	...
	If (strlen($STYLE) < 8) {
		texto($STYLE, 10, 64, 0, 0, $arialBold, $black, 7, 0, 0);
	} else {
		texto($STYLE, 10, 64, 0, 0, $arialNarrowBold, $black, 7, 0, 0);
	}

	If (strlen($COLOR) < 8) {
		texto($COLOR, 0, 64, 2, 10, $arialBold, $black, 7, 0, 0);
	} else {
		texto($COLOR, 0, 64, 2, 10, $arialNarrowBold, $black, 7, 0, 0);
	}

	If (strlen($DESCRIPTION) < 20) {
		texto($DESCRIPTION, 8, 78, 1, 0, $arialBold, $black, 7, 0, 0);
	} else {
		texto($DESCRIPTION, 8, 78, 1, 0, $arialNarrowBold, $black, 7, 0, 0);
	}
